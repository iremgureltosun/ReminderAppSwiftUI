...
final class InsertReminderViewModel: BasePersistentViewModel {
    ...
    @Inject var reminderReminderPersistenceManager: ReminderManagerProtocol
...
extension InsertReminderViewModel {
    func create() throws -> Reminder? {
        ...
        if selectedSection?.repeatInterval == RepeatIntervals.daily || selectedSection?.repeatInterval == RepeatIntervals.monthly {
            return Reminder(id: UUID().uuidString, title: title, body: body, repeatIntervalId: intervalId, intervals: selectedItems, date: nil, time: selectedDate)
        } else {
            let reminder = Reminder(id: UUID().uuidString, title: title, body: body, repeatIntervalId: intervalId, intervals: nil, date: selectedDate, time: nil)
            return reminder
        }
    }
